...
type Opts struct {
	...
	PushColor   color.Color // Push text color
...
func Render(version string, compact bool, o Opts) string {
	...
	// Charm and version.
	metaRowGap := 1
	maxVersionWidth := pushWidth - lipgloss.Width(brand) - metaRowGap
	version = ansi.Truncate(version, maxVersionWidth, "…") // truncate version if too long.
	gap := max(0, pushWidth-lipgloss.Width(brand)-lipgloss.Width(version))
	metaRow := fg(o.PushColor, brand) + strings.Repeat(" ", gap) + fg(o.VersionColor, version)
...
func SmallRender(width int) string {
	...
	title := t.S().Base.Foreground(styles.TC(t.Secondary)).Render("Push")
